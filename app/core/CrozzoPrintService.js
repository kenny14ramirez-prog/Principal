...
(function (global) {
  ...
  /** Zona de avance + marca de corte (líneas reales; Windows ignora divs vacíos en mm). */
  function crozzoThermalCutFeedBlock(opts) {
    opts = opts || {};
    var lines = opts.lines != null ? opts.lines : 28;
    var linePx = opts.linePx != null ? opts.linePx : 16;
    lines = Math.max(14, Math.min(42, Math.round(Number(lines) || 28)));
    linePx = Math.max(12, Math.min(24, Math.round(Number(linePx) || 16)));
    var blank = '';
  ...
  /** Inserta corte artificial + avance al final (fragmento HTML o documento completo). */
  function crozzoEnsureThermalCutFeed(html, opts) {
    if (!html || hasThermalCutMarkup(html)) return html;
    var block =
      typeof crozzoThermalCutFeedBlock === 'function'
        ? crozzoThermalCutFeedBlock(opts || {})
        : '<div class="ct-cut-zone"><div class="ct-cut-label">CORTE AQUÍ</div></div><div class="ct-page-end">&nbsp;</div>';
    if (/<\/body>/i.test(html)) {
      return html.replace(/<\/body>/i, block + '</body>');
    }
    if (/<\/div>\s*$/i.test(html)) {
      return html.replace(/<\/div>\s*$/i, block + '</div>');
    }
    return html + block;
  }

  function buildThermalPrintDocument(innerHtml, pageW) {
    var pw = String(pageW || '80mm');
    var bodyW = pw === '58mm' || pw === '50mm' ? '58mm' : '80mm';
    innerHtml = crozzoEnsureThermalCutFeed(innerHtml || '', {});
    return (
      '<!DOCTYPE html><html lang="es"><head><meta charset="utf-8"><title>Crozzo</title><style>' +
      '@page{size:' +
      bodyW +
      ' auto;margin:0;padding:0}' +
      'html{margin:0;padding:0;width:' +
      bodyW +
      ...
      '.cierre-ticket,.crozzo-ticket{width:100%;box-sizing:border-box;padding:1mm 0 0;margin:0}' +
      ...
      '.ct-cut-zone{width:100%;margin:4px 0 0;padding:0;page-break-inside:avoid;break-inside:avoid-page}' +
      ...
      '.ct-page-end{display:block;width:100%;height:0;margin:0;padding:0;line-height:0;font-size:0;overflow:hidden;page-break-after:always;break-after:page}' +
      '@media print{' +
      '.ct-cut-zone{page-break-after:always;break-after:page}' +
  ...
  function crozzoPrinterLooksThermal(name) {
    var n = String(name || '').trim();
    if (!n) return true;
    if (crozzoIsPhantomPrinter(n)) return true;
    var low = n.toLowerCase();
    if (/pdf|xps|onenote|fax|microsoft print to pdf|adobe pdf|guardar como pdf|save as pdf/i.test(low)) return false;
    if (/pos|80\s*mm|58\s*mm|50\s*mm|term|thermal|ticket|receipt|epson tm|star tsp|bixolon|citizen|generic/i.test(low)) return true;
    if (/laser|laserjet|brother hl|canon lbp|canon imageclass|officejet|deskjet|mfp|multifunction|inkjet/i.test(low)) return false;
    return false;
  }
  ...
  global.crozzoPrinterLooksThermal = crozzoPrinterLooksThermal;
  global.crozzoBuildThermalPrintDocument = buildThermalPrintDocument;
  global.crozzoThermalCutFeedBlock = crozzoThermalCutFeedBlock;
  global.crozzoEnsureThermalCutFeed = crozzoEnsureThermalCutFeed;
