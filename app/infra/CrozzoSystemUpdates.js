...
(function (global) {
  ...
  function startCriticalDownloadForRestart(entry, opts) {
    ...
    var preLogin = crozzoUpdateIsPreLogin();
    _criticalDownloadSilent = !!opts.silent && !preLogin;
    ...
    return prefetchUpdateArtifactForRestart(entry, function (p) {
      ...
      .then(function (meta) {
        ...
        syncLoginUpdateBanner({ mode: 'pending_restart', entry: entry });
        if (typeof global.showToast === 'function' && (shouldShowCriticalOverlay(entry) || preLogin)) {
  ...
  var _postUpdateWelcomePack = null;
  ...
  function markEntryFullyApplied(entry, targetVersion, opts) {
    ...
    queuePostUpdateWelcome(entry, tv, previousVer);
  ...
  function crozzoUpdateIsPreLogin() {
    try {
      var u = typeof global.getCurrentUser === 'function' ? global.getCurrentUser() : null;
      return !u;
    } catch (_) {
  ...
  global.crozzoUpdateIsPreLogin = crozzoUpdateIsPreLogin;

  function ensureLoginUpdateBanner() {
    var el = document.getElementById('loginUpdateBanner');
    if (el) return el;
    var anchor = document.getElementById('loginSubtitle');
    if (!anchor || !anchor.parentNode) return null;
    el = document.createElement('div');
    el.id = 'loginUpdateBanner';
    el.className = 'login-update-banner';
    el.setAttribute('role', 'status');
    el.setAttribute('aria-live', 'polite');
    el.hidden = true;
    el.innerHTML =
      '<span class="login-update-banner__icon" aria-hidden="true"><i data-lucide="refresh-cw"></i></span>' +
      '<div class="login-update-banner__body">' +
      '<strong id="loginUpdateBannerTitle">Actualizando el sistema</strong>' +
      '<p id="loginUpdateBannerMsg">Comprobando actualizaciones…</p></div>';
    anchor.parentNode.insertBefore(el, anchor.nextSibling);
    refreshUpdateIcons();
    return el;
  }

  function syncLoginUpdateBanner(opts) {
    opts = opts || {};
    if (opts.mode === 'hide') {
      var hideEl = document.getElementById('loginUpdateBanner');
      if (hideEl) hideEl.hidden = true;
      return;
    }
    if (!crozzoUpdateIsPreLogin() && !opts.force) return;
    var banner = ensureLoginUpdateBanner();
    if (!banner) return;
    var titleEl = document.getElementById('loginUpdateBannerTitle');
    var msgEl = document.getElementById('loginUpdateBannerMsg');
    var mode = opts.mode || '';
    var entry = opts.entry || _pendingCriticalEntry || null;
    var ver =
      opts.version ||
      (entry ? normEntryVersion(entry) : '') ||
      VERSION_AVAIL ||
      VERSION;
    ...
    if (!mode) {
      if (_criticalInstallState === 'downloading' || _criticalInstallState === 'installing') {
        mode = 'busy';
      } else if (_criticalInstallState === 'pending_restart') {
    ...
    banner.hidden = false;
    banner.classList.remove('is-critical', 'is-busy', 'is-applied');
    if (mode === 'busy') {
      banner.classList.add('is-critical', 'is-busy');
      if (titleEl) titleEl.textContent = 'Actualizando el sistema';
      if (msgEl) {
        msgEl.textContent =
          opts.message ||
          _installUi.message ||
    ...
    if (mode === 'applied') {
      ...
      if (titleEl) titleEl.textContent = 'Sistema actualizado';
      if (msgEl) {
        msgEl.textContent =
          'Versión ' +
          ver +
          ' instalada. Inicie sesión para ver el resumen de cambios.';
      }
      return;
    }
    if (mode === 'optional') {
      banner.classList.remove('is-critical');
      if (titleEl) titleEl.textContent = 'Mejora disponible (opcional)';
      if (msgEl) {
        msgEl.textContent =
          'Versión ' +
          ver +
          ' lista para instalar. Tras iniciar sesión podrá aceptarla o posponerla.';
      }
      return;
    }
    if (mode === 'checking') {
  ...
  /** Mensaje/changelog del registry → ítems de lista (1 por cambio). */
  function parseChangelogMessage(raw) {
    if (raw == null) return [];
  ...
  function normalizeEntryChangelog(entry) {
    if (!entry) return [];
    if (Array.isArray(entry.changelog) && entry.changelog.length) {
      return parseChangelogMessage(entry.changelog);
    }
    if (entry.message) return parseChangelogMessage(entry.message);
    return [];
  }
  ...
  function resolvePostUpdateChanges(pack) {
    pack = pack || {};
    if (pack.changes && pack.changes.length) {
      return parseChangelogMessage(pack.changes);
    }
    var entry = null;
    if (pack.entryId) {
      entry =
        (_registryEntries || []).find(function (e) {
          return entryId(e) === pack.entryId;
        }) || null;
    }
    if (!entry && pack.version) {
      var target = normEntryVersion({ version: pack.version });
      entry =
        (_registryEntries || []).find(function (e) {
          return normEntryVersion(e) === target;
        }) || null;
    }
    var lines = normalizeEntryChangelog(entry);
    if (lines.length) return lines;
    if (pack.message) return parseChangelogMessage(pack.message);
    return [];
  }

  function queuePostUpdateWelcome(entry, targetVersion, previousVersion) {
    var changes = normalizeEntryChangelog(entry);
    if (!changes.length && entry && entry.message) {
      changes = parseChangelogMessage(entry.message);
    }
    var ctx = getUpdateOperativeContext();
    var ver = targetVersion || normEntryVersion(entry);
    var headline =
      ctx.experiencia === 'novice'
        ? 'Listo — la app quedó actualizada. Siga con su turno con normalidad.'
        : 'Actualización ' + ver + ' aplicada correctamente.';
    try {
      localStorage.setItem(
        ...
        JSON.stringify({
          at: Date.now(),
          version: ver,
          previous: previousVersion || packPreviousFromEntry(entry, ver),
          entryId: entry ? entryId(entry) : '',
  ...
  function packPreviousFromEntry(entry, ver) {
    try {
      var idx = (_registryEntries || []).findIndex(function (e) {
        return entry && entryId(e) === entryId(entry);
      });
      if (idx > 0) return normEntryVersion(_registryEntries[idx - 1]);
    } catch (_) {}
    return ver ? String(ver).replace(/(\d+)$/, function (m) {
      return String(Math.max(0, parseInt(m, 10) - 1));
    }) : '';
  }
  ...
  function crozzoDismissPostUpdateWelcome() {
    _postUpdateWelcomePack = null;
    if (_criticalInstallState === 'welcome') _criticalInstallState = 'idle';
    setCriticalOpen(false);
  }

  function maybeShowPostUpdateWelcome(forceAfterLogin) {
    if (!forceAfterLogin && crozzoUpdateIsPreLogin()) return;
    var pack = null;
    try {
      var raw = localStorage.getItem(LS_POST_UPDATE_WELCOME);
    ...
    syncLoginUpdateBanner({ mode: 'hide', force: true });
    setTimeout(function () {
      showPostUpdateWelcomeOverlay(pack);
    }, forceAfterLogin ? 600 : 0);
  }
  ...
  function scheduleCriticalInstallRetry(entry, err) {
    ...
    if (crozzoUpdateUserBusy() || posIsOperationBusy()) {
      ...
      if (typeof global.showToast === 'function') {
        global.showToast(
          'Actualización crítica pendiente. La caja sigue operando; se aplicará al cerrar el turno.',
          'info'
        );
      }
    } else {
  ...
  function handleInstallProgress(p) {
    ...
    if (
      crozzoUpdateIsPreLogin() &&
      (_criticalInstallState === 'installing' || _criticalInstallState === 'downloading')
    ) {
      syncLoginUpdateBanner({ mode: 'busy', message: p.message || _installUi.message });
    }
  }
  ...
  function populateCriticalInfo(state, errMsg) {
    ...
    if (state === 'info') {
      ...
      if (title) title.textContent = 'BONA origen está al día';
      if (lead) {
        lead.textContent =
          'Versión ' +
          (info.version || VERSION) +
          ' instalada. Esto es lo que cambió en esta actualización:';
      }
      if (dismiss) {
        dismiss.disabled = false;
        dismiss.textContent = 'Entendido — continuar';
      }
      if (retry) retry.style.display = 'none';
      var planBWelcome = document.getElementById('crozzoUpdateCriticalPlanB');
      if (planBWelcome) planBWelcome.style.display = 'none';
      var laterWelcome = document.getElementById('crozzoUpdateCriticalLater');
      if (laterWelcome) laterWelcome.style.display = 'none';
      var progWelcome = document.getElementById('crozzoUpdateCriticalProgress');
      if (progWelcome) progWelcome.hidden = true;
    ...
    if (list) {
      var lines = parseChangelogMessage(info.installed || []);
    ...
    setTimeout(function () {
      ...
      if (_deferredAndroidCritical) {
        ...
        if (
          ...
          syncLoginUpdateBanner({ mode: 'pending_restart', entry: deferredEntry });
          return;
        }
  ...
  function setBootGateMessage(msg) {
    ...
    setCheckStatus(msg || '');
    if (crozzoUpdateIsPreLogin()) {
      syncLoginUpdateBanner({ mode: 'checking', message: msg, force: true });
    }
  }
  ...
  function shouldRunCriticalInstallNow() {
    if (crozzoUpdateIsPreLogin()) return true;
    if (crozzoUpdateUserBusy() || posIsOperationBusy()) return false;
    return true;
  }

  function isReleaseNotReadyError(err) {
    var msg = String((err && err.message) || err || '').toLowerCase();
    return (
      /pequeñ|pequeno|corrupto|archivo_pequeno|demasiado pequeño/.test(msg) ||
      /github actions|aún no está|todavía no|no se encontró el instalador|sin instalador/.test(msg) ||
      /espere github|compilando en la nube|release exista/.test(msg)
    );
  }

  function deferCriticalUntilReleaseReady(entry, opts) {
    opts = opts || {};
    if (!entry) return Promise.resolve({ deferred: true });
    _pendingCriticalEntry = entry;
    _currentCriticalId = entryId(entry);
    _criticalAutoAttempts = 0;
    _installInProgress = false;
    ...
    if (!opts.quiet && typeof global.showToast === 'function') {
      try {
        global.showToast(
          'Actualización crítica en preparación en la nube. Siga operando; se aplicará sola cuando el instalador esté listo.',
          'info'
        );
      } catch (_) {}
    }
  ...
  function runBootCriticalInstallLoop() {
    ...
    var profile = getUpdateClientProfile();
    if (shouldDeferCriticalAutoOnBoot(profile) && !crozzoUpdateIsPreLogin()) {
      beginCriticalEntryInstall(entry, { returnPromise: true, forceAuto: false, deferOverlay: true });
    ...
    return beginCriticalEntryInstall(entry, {
      ...
      if (res && (res.pendingRestart || res.deferred)) return res;
      return refreshBinaryVersion().then(function () {
  ...
  function finishBootUpdatePipeline(detail) {
    syncLoginUpdateBanner();
    if (!_bootUpdatesReady) {
      notifyBootUpdatesReady(detail || { ok: true, reason: 'non_blocking' });
    }
  }
    ...
    var pipeline = refreshBinaryVersion()
      ...
      .finally(function () {
        ...
        if (
          ...
          waitForBootCriticalWorkThenFinish({ ok: true, reason: 'await_critical' });
          return;
        }
        hideBootUpdateGate();
    ...
    return applyClientUpdate(remote, null, {
      ...
      .catch(function (err) {
        ...
        if (crozzoUpdateUserBusy() || posIsOperationBusy()) {
          ...
          if (typeof global.showToast === 'function') {
            global.showToast(
              'Actualización pendiente — siga operando. Se reintentará cuando la caja esté libre.',
              'info'
            );
          }
        } else if (!/método alternativo/i.test(msg) && !_bootUpdatePhase) {
          setCriticalOpen(true);
          populateCriticalInfo('failed', msg);
  ...
  function showBuildOnlyUpdate(entry, opts) {
    ...
    UPDATE_CRITICAL_INSTALLED = {
      ...
      previous: VERSION,
      date: formatManifestDate(entry.publishedAt),
      installed: normalizeEntryChangelog(entry),
    };
    setNormalOpen(false);
    hideBootUpdateGate();
  ...
  function beginCriticalEntryInstall(entry, opts) {
    ...
    var silentDl = !crozzoUpdateIsPreLogin() || userBusy;
  ...
  function showOptionalEntry(entry) {
    ...
    setCriticalOpen(false);
    if (crozzoUpdateIsPreLogin()) {
      syncLoginUpdateBanner({ mode: 'optional', entry: entry, version: remote });
      setCheckStatus('Mejora ' + remote + ' disponible. Instálela tras iniciar sesión.');
      return true;
    }
  ...
  function crozzoCerrarActualizacionCritica() {
    if (_criticalInstallState === 'welcome') {
      crozzoDismissPostUpdateWelcome();
      return;
    }
    if (_criticalInstallState === 'pending_restart') {
  ...
  function showPendingOptionalAfterSession() {
    if (_installInProgress || _installUi.open) return;
    if (
      _criticalInstallState === 'installing' ||
      _criticalInstallState === 'downloading' ||
      _criticalInstallState === 'pending_restart'
  ...
  function onAuthReady() {
    ...
    setTimeout(function () {
      maybeShowPostUpdateWelcome(true);
      runCloudSyncAfterAppUpdateIfPending();
      if (_pendingCriticalEntry && _criticalInstallState === 'pending_restart') {
        if (shouldShowCriticalOverlay(_pendingCriticalEntry)) {
          setCriticalOpen(true);
          populateCriticalInfo('pending_restart');
        } else if (typeof global.showToast === 'function') {
          global.showToast(
            'Actualización crítica ' +
              normEntryVersion(_pendingCriticalEntry) +
              ' lista. Se aplicará al reiniciar la aplicación.',
            'info'
          );
        }
      }
      showPendingOptionalAfterSession();
    }, 1500);
  }
  ...
  function startCrozzoUpdateChecks() {
    fetchTauriBinaryVersion().then(function (binaryVer) {
      ...
      setCheckStatus('Versión ' + VERSION + ' · actualizaciones revisadas al iniciar la app.');
  ...
  function boot() {
    ...
    global.addEventListener('crozzo:boot-updates-ready', function () {
      syncLoginUpdateBanner();
    });
    setTimeout(function () {
      syncLoginUpdateBanner();
