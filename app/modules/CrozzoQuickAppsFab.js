/**
 * FAB "+" — despliega insignias WhatsApp, Gmail y Drive (solo PC Tauri).
 */
(function (global) {
  'use strict';
  ...
  function markActiveBadge(active) {
    ['crozzoQuickAppsWaBtn', 'crozzoQuickAppsGmailBtn', 'crozzoQuickAppsDriveBtn'].forEach(function (id) {
      var btn = document.getElementById(id);
      if (!btn) return;
      btn.classList.remove('crozzo-quick-apps-fab__badge--current');
    });
    var map = { wa: 'crozzoQuickAppsWaBtn', gmail: 'crozzoQuickAppsGmailBtn', drive: 'crozzoQuickAppsDriveBtn' };
    if (active && map[active]) {
      var cur = document.getElementById(map[active]);
  ...
  function refreshFab() {
    ...
    if (!stack) return;
    var active = getActiveEmbed();
    var show = isDesktopPc() && !isLoginOpen() && !active;
    if (show) {
