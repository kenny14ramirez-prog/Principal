...
call :mirror_scripts "%CROZZO_ROOT%\scripts" "%CROZZO_STAGING%\scripts"

if exist "%CROZZO_ROOT%\Herramientas Crozzo POS.bat" (
  copy /Y "%CROZZO_ROOT%\Herramientas Crozzo POS.bat" "%CROZZO_STAGING%\Herramientas Crozzo POS.bat" >nul
)
...
call :mirror_scripts "%CROZZO_STAGING%\scripts" "%CROZZO_WT%\scripts"

if exist "%CROZZO_STAGING%\Herramientas Crozzo POS.bat" (
  copy /Y "%CROZZO_STAGING%\Herramientas Crozzo POS.bat" "%CROZZO_WT%\Herramientas Crozzo POS.bat" >nul
)
...
git add releases\ releases\*.json app\ src\ src-tauri\ scripts\
if exist "Herramientas Crozzo POS.bat" git add "Herramientas Crozzo POS.bat"
...
robocopy "%SRC_DIR%" "%DST_DIR%" /E /XD _qa-out node_modules /XF *.log _training-* _update-audit-report.json /NFL /NDL /NJH /NJS /nc /ns /np >nul
exit /b 0
